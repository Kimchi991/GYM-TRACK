...
$apiProcess = Start-Process dotnet -ArgumentList "run --project src/GymTrackPro.API" -WorkingDirectory $PWD -PassThru -NoNewWindow -RedirectStandardOutput "api_auth_integration_test.log" -RedirectStandardError "api_auth_integration_test_error.log"
...
Assert-Test "Login with Verified Credentials" {
    ...
    $response.success -eq $true -and $response.data.token -ne $null -and ($response.data.role -eq "PlatformAdmin" -or $response.data.role -eq "Administrator")
}
...
Assert-Test "Role Authorization Restrictions" {
    ...
    try {
        ...
        if ($_.Exception.Response) {
            Write-Host "Admin Exception StatusCode: $($_.Exception.Response.StatusCode)" -ForegroundColor DarkYellow
            $stream = $_.Exception.Response.GetResponseStream()
            $reader = New-Object System.IO.StreamReader($stream)
